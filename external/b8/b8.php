...
class b8
{
	...
	const LEXER_FAILED          = 'LEXER_FAILED';
	...
	function __construct($config = array(), $database_config)
	{
			...
			foreach ($config as $name=>$value) {
				...
				switch($name) {
					...
					case 'lexer':
						...
						self::$config[$name] = is_file(dirname(__FILE__) . DIRECTORY_SEPARATOR . 'lexer' . DIRECTORY_SEPARATOR . "lexer_" . $value . '.php') === TRUE ? $value : 'default';
						break;

					case 'storage':
						self::$config[$name] = (string) $value;
						break;
		...
		if(class_exists('b8_storage_' . self::$config['storage'], FALSE) === FALSE) {

			$included = require_once dirname(__FILE__) . DIRECTORY_SEPARATOR . 'storage' . DIRECTORY_SEPARATOR . 'storage_' . self::$config['storage'] . '.php';

			if($included === FALSE or class_exists('b8_storage_' . self::$config['storage'], FALSE) === FALSE)
				return;

		}

		$class = 'b8_storage_' . self::$config['storage'];
		$this->_database = new $class($database_config);

		# Setup the lexer class

		if(class_exists('b8_lexer_' . self::$config['lexer'], FALSE) === FALSE) {

			$included = require_once dirname(__FILE__) . DIRECTORY_SEPARATOR . 'lexer' . DIRECTORY_SEPARATOR . 'lexer_' . self::$config['lexer'] . '.php';

			if($included === FALSE or class_exists('b8_lexer_' . self::$config['lexer'], FALSE) === FALSE)
				return;

		}

		$class = 'b8_lexer_' . self::$config['lexer'];
	...
	public function classify($text)
	{
		...
		for($i = 0; $i < self::$config['use_relevant']; $i++) {
			...
			if($tmp = each($importance)) {
				...
				if(abs(0.5 - $rating[$tmp['key']]) > self::$config['min_dev']) {
		...
		# Alea iacta est
		return str_replace(",", ".", $probability);
	...
	private function _calc_probability($data, $texts_ham, $texts_spam)
	{
		...
		if($texts_ham > 0)
			$data['count_ham'] = $data['count_ham'] / $texts_ham;

		if($texts_spam > 0)
			$data['count_spam'] = $data['count_spam'] / $texts_spam;

		$rating = $data['count_spam'] / ($data['count_ham'] + $data['count_spam']);

		# Calculate the better probability proposed by Mr. Robinson
		$all = $data['count_ham'] + $data['count_spam'];
		return ((self::$config['rob_s'] * self::$config['rob_x']) + ($all * $rating)) / (self::$config['rob_s'] + $all);
