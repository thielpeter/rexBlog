...
# Get the basic functions used by all storage backends
require_once dirname(__FILE__) . DIRECTORY_SEPARATOR . "storage_base.php";
...
class b8_storage_mysql extends b8_storage_base
{
					...
					case 'connection':
						...
						if($value !== NULL) {
							...
							if(is_resource($value) === TRUE) {
								...
								self::$config[$name] = $resource_type !== 'mysql link' && $resource_type !== 'mysql link persistent' ? FALSE : $value;
							}
	...
	public function connect()
	{
		...
		# Are we using a different resource?
		if(self::$config['connection'] === FALSE) {
			# ... yes we are, but the connection is not a resource, so return an error
			$this->connected = FALSE;
			return self::DATABASE_CONNECTION_BAD_RESOURCE;
		}

		elseif(self::$config['connection'] === NULL) {

			# ... no we aren't so we must connect.

			if($this->_connection = mysql_connect(self::$config['host'], self::$config['user'], self::$config['pass'])) {
				if(mysql_select_db(self::$config['database'], $this->_connection) === FALSE) {
					$this->connected = FALSE;
			...
			# ... yes we are
			$this->_connection = self::$config['connection'];
		}
		...
		if(mysql_query('DESCRIBE ' . self::$config['table_name'], $this->_connection) === FALSE) {
			$this->connected = FALSE;
	...
	protected function _commit()
	{
		...
		if(count($this->_puts) > 0) {
			...
			$result = mysql_query('
				INSERT INTO ' . self::$config['table_name'] . '(token, count)
		...
		if(count($this->_updates) > 0) {
			...
			$result = mysql_query('
				...
				ON DUPLICATE KEY UPDATE ' . self::$config['table_name'] . '.count = VALUES(count);', $this->_connection);
