...
abstract class b8_storage_base
{
	...
	const DEGENERATOR_FAILED     = 'DEGENERATOR_FAILED';
	...
	protected function validate()
	{
		...
		$class = 'degenerator_' . b8::$config['degenerator'];

		if(class_exists("b8_$class", FALSE) === FALSE) {

			$included = require_once dirname(__FILE__) . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'degenerator' . DIRECTORY_SEPARATOR . "$class.php";

			if($included === FALSE or class_exists("b8_$class", FALSE) === FALSE)
				return self::DEGENERATOR_FAILED;
	...
	public function get($tokens)
	{
		...
		foreach($tokens as $token) {
			...
			if(isset($token_data[$token]) === TRUE) {
				...
				$this->_update($token, "{$return_data_tokens[$token]['count_ham']} {$return_data_tokens[$token]['count_spam']} " . b8::$today);
				...
				foreach($this->_degenerator->degenerates[$token] as $degenerate) {
					...
					if(isset($token_data[$degenerate]) === TRUE) {
						...
						# ... and update it's lastseen parameter
						$this->_update($degenerate, "{$return_data_degenerates[$token][$degenerate]['count_ham']} {$return_data_degenerates[$token][$degenerate]['count_spam']} " . b8::$today);
		...
		foreach($tokens as $token => $count) {
			...
			if(isset($token_data[$token])) {
				...
				if($count_ham !== 0 or $count_spam !== 0)
					$this->_update($token, "$count_ham $count_spam " . b8::$today);
				else
					$this->_del($token);
