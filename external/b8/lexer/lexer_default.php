...
class b8_lexer_default
{
	...
	public function get_tokens($text)
	{
		...
		preg_match_all(self::$regexp['ip'], $text, $raw_tokens);
		...
		foreach($raw_tokens[1] as $word) {
			...
			if(strpos($word, '.') !== FALSE) {
				...
				if($this->_is_valid($word) === TRUE) {
					...
					$url_parts = preg_split(self::$regexp['raw_split'], $word);
		...
		$raw_tokens = preg_split(self::$regexp['raw_split'], $text);
		...
		preg_match_all(self::$regexp['html'], $text, $raw_tokens);
		...
		foreach($raw_tokens[1] as $word) {
			...
			if($this->_is_valid($word) === TRUE) {
				...
				if(strpos($word, ' ') !== FALSE) {
					preg_match(self::$regexp['tagname'], $word, $tmp);
					$word = "{$tmp[1]}...>";
				}
	...
	private function _is_valid($token)
	{
		...
		if($len < b8::$config['min_size'] or $len > b8::$config['max_size'])
			return FALSE;

		# We may want to exclude pure numbers
		if(b8::$config['allow_numbers'] === FALSE) {
			if(preg_match(self::$regexp['numbers'], $token) > 0)
				return FALSE;
		}
